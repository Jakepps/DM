...
divided_arrs = ((8,1,1,1),(7,2,1,1),(6,3,1,1),(6,2,2,1),(5,4,1,1),(5,3,2,1),(5,2,2,2),(4,3,2,2),(4,3,3,1),(4,3,2,2))
    ...
    def next(self,pos_r=None):
        ...
        if idx < 0:
            ...
            if self.pos_l:
                ...
                # для самой левой (старшей) позиции, т.е. перебор закончен
                self.word.stop()
                return False
        else:
            self.cur_ind_c = idx
...
class Word:
    ...
    def init_pos(self):
        ...
        if self.len_word == 0:
            # генерим максимальное возможное
            s = 0
...
def main(f):
    g_cnt = 0
    cc_cnt = 0
    # Сперва получим все сочетания из 12 по 5
    for ccc in combinations(d,5):
        ...
        for divide_arr in divided_arrs:
            ...
            # получаем комбинации букв по кол-ву значимых позиций и генерим через перестановки с повторениями поток в файл
            for dd in get_uniq_cnt(ccc,divide_arr):
                str_var_c = ''.join(ccc)
                for y in permut_word(str_var_c, dd):
                    f.write(str(y) + '\n')
                    g_cnt = g_cnt + 1
            #проверка на ашалевшие числа  
        if g_cnt > 10**7:
